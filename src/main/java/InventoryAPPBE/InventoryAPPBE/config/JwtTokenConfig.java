...
public class JwtTokenConfig implements Serializable {
    ...
    public void setJwtTokenValidity(int JWT_TOKEN_VALIDITY) {
    ...
    private String doGenerateToken(Map<String, Object> claims, String subject) {
        ...
        Date afterAddingMins = new Date(timeInSecs + ((long) JWT_TOKEN_VALIDITY * 60 * 1000));

        return Jwts.builder().setClaims(claims).setSubject(subject).setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(afterAddingMins)
    ...
    public Boolean validateToken(String token, User userDetails) {
        ...
        return Integer.parseInt(id) == userDetails.getId() && !isTokenExpired(token);
    }
}
